...
class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("Pisos", "assets/bloques.png"); // Donde están los patrones
        this.load.tilemapTiledJSON("tilemap", "assets/Nivel-Rene.json"); // Archivo JSON de Tiled
    
    }

    create() {
        let fondo = this.add.image(0, 0, "bg").setOrigin(0.0);
        fondo.setScale(0.15); // Escalar el fondo para que se ajuste al mundo

        this.physics.world.setBounds(0, 0, 8040, 3128); // Establecer límites del mundo del juego

        // Cargar el mapa de Tiled
        const map = this.make.tilemap({ key: "tilemap" });
        const tileset = map.addTilesetImage("bloques", "Pisos"); // Nombre del tileset en Tiled

        const suelo = map.createLayer("CapaBase", tileset); // Capa del suelo
        suelo.setCollisionByProperty({ colision: true });
    ...
    update() {
        // Reiniciar la velocidad antes de asignarla
...
const config = {
    ...
    physics: {
        default: "arcade",
        arcade: {
            gravity: { y: 100 }, // Activar gravedad para objetos físicos
            debug: true
        }
    },
    scene: [GameScene]
